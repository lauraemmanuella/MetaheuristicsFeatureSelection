...
public class Individuo implements Comparable<Individuo> {
     ...
     * @throws Exception
     */
    public Individuo(int qtdAtributos) throws Exception {
    ...
    private void iniciarGenes() throws Exception {
        System.out.println("genes" + genes.length);

        for (int i = 0; i < genes.length; i++) {
            double vai = Math.random();
            if (vai < 0.5) {
                genes[i] = 1;
                //System.out.println("Y"+vai);
            } else {
                genes[i] = 0;
                //System.out.println(/*"N"+getAtributos()+"     COM: "+*/vai);
            }
        }
    }
    ...
    public ArrayList<Integer> getAtributos() {
        ...
        for (int i = 0; i < genes.length; i++) {
            if (genes[i] == 0) {
                ...
                //System.out.println("atributo a ser removido " + atributos);
            }
        }
        return atributos;
    }
    ...
    /**
     * caso a aptidao de dois individuos seja igual, ele prioriza o que
     * selecionou mais atributos
     *
     * @param i
     * @return
     */
